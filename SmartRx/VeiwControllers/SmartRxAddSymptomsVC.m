...
#import "SmartRxAddSymptomsVC.h"
#import "SmartRxDashBoardVC.h"

@interface SmartRxAddSymptomsVC ()
{
    CGSize viewSize;
...
@implementation SmartRxAddSymptomsVC
...
- (void)viewDidLoad {
    [super viewDidLoad];
    [[SmartRxCommonClass sharedManager] setNavigationTitle:@"Add Symptom" controler:self];
    [self.tableView setTableFooterView:[UIView new]];
    self.tableView.hidden = YES;
    self.symptomNameTF.delegate = self;
    [self.symptomNameTF addTarget:self
...
-(void)textFieldDidChange :(UITextField *)textField{
    ...
    self.searchTimer = [NSTimer scheduledTimerWithTimeInterval: 1.0
                                                      ...
                                                      userInfo: self.symptomNameTF.text
                                                       repeats: NO];
...
- (void)searchForKeyword:(NSTimer *)timer
{
    ...
    [self.symptomNameTF resignFirstResponder];
    if (self.symptomNameTF.text.length > 0) {
...
-(IBAction)clickOnSaveBtn:(id)sender{
    if (self.symptomNameTF.text.length < 1) {
        [self customAlertView:@"" Message:@"Symptom name is required.." tag:0];
        ...
        NSDictionary *dict = [NSDictionary dictionaryWithObjectsAndKeys:@"r_symptom",@"record_type",symptomId,@"a_symptom_id",startDateStr,@"observed_at",self.symptomNameTF.text,@"a_symptom_name",userId,@"member_id",@"",@"entry_id", nil];
    ...
    [[SmartRxCommonClass sharedManager] addEHR:info successHandler:^(id response) {
        ...
        dispatch_async(dispatch_get_main_queue(),^{
            ...
            if ([response[@"result"] integerValue] == 1) {
                [self customAlertView:@"" Message:@"Symptom added successfully" tag:999];
            } else {
                   [self customAlertView:@"Error" Message:@"EHR updation failed .Please try again later." tag:0];
            }
            NSLog(@"response date:%@",response);
        });
...
-(void)makeRequestForSymptomsList{
    ...
    NSString *url=[NSString stringWithFormat:@"%s/ehr/archetypes/a_symptom/search/%@?count=100",kBaseUrl,self.symptomNameTF.text];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    cell.textLabel.text = self.symptomsList[indexPath.row][@"symptom_name"];
    return cell;
}
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    selectedSymptom = self.symptomsList[indexPath.row];
    self.symptomNameTF.text = self.symptomsList[indexPath.row][@"symptom_name"];
    self.tableView.hidden = YES;

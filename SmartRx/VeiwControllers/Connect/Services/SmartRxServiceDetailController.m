//
//  SmartRxServiceDetailController.m
//  SmartRx
//
...
@interface SmartRxServiceDetailController ()<UITableViewDataSource,UITableViewDelegate>

@property(nonatomic,strong) ServicesResponseModel *selectedService;

@end

@implementation SmartRxServiceDetailController
...
//            NSAttributedString *str = [self getPlainText:self.selectedService.serviceDescription];
//            descriptionLbl.text = str.string;
            descriptionLbl.attributedText =[self getPlainText:self.selectedService.serviceDescription];
              descriptionLbl.numberOfLines = 1000;
            NSLog(@"hjdethr:%f",descriptionLbl.frame.size.height);
            NSLog(@"hjdethr:%f",cell.frame.size.height);
        } else {
            descriptionLbl.text = @"";
        }
        ...
        //instructionLbl.text = self.selectedService.instructions;
...
-(IBAction)clickOnCancelButton:(id)sender{
   ...
   [self.buttonDelegate cancelButtonClicked];
